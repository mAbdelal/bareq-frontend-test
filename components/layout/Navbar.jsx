...
import { Briefcase, Layers, Users, LogOut, User, Home, Key, Menu, Bell } from "lucide-react";
...
export default function Navbar() {
    ...
    const markAsRead = async (id) => {
        try {
            await fetchWithAuth(`${process.env.NEXT_PUBLIC_BASE_URL}/notifications/${id}/mark-as-read`, { method: "PATCH" });
    ...
    const markAllAsRead = async () => {
        try {
            await fetchWithAuth(`${process.env.NEXT_PUBLIC_BASE_URL}/notifications/mark-as-read`, { method: "PATCH", credentials: "include" });
        ...
        <nav className="bg-background text-label w-full shadow-md fixed z-50 top-0">
            <Container className="px-4 sm:px-6 lg:px-8 py-3 flex justify-between items-center">
                ...
                <ul className="hidden md:flex items-center gap-8">
                    <li>
                        <Link href="/services" className={`flex items-center gap-2 font-bold relative ${isActive("/services") ? "text-primary" : "hover:text-hover-label"}`}>
                            <Briefcase className="w-5 h-5" />
                            خدمات
                            {isActive("/services") && <span className="absolute -bottom-1 left-0 right-0 h-[2px] bg-primary rounded-full"></span>}
                        </Link>
                    </li>
                    <li>
                        <Link href="/requests" className={`flex items-center gap-2 font-bold relative ${isActive("/requests") ? "text-primary" : "hover:text-hover-label"}`}>
                            <Layers className="w-5 h-5" />
                            طلبات
                            {isActive("/requests") && <span className="absolute -bottom-1 left-0 right-0 h-[2px] bg-primary rounded-full"></span>}
                        </Link>
                    </li>
                    <li>
                        <Link href="/academics" className={`flex items-center gap-2 font-bold relative ${isActive("/academics") ? "text-primary" : "hover:text-hover-label"}`}>
                            <Users className="w-5 h-5" />
                            أكاديميين
                            {isActive("/academics") && <span className="absolute -bottom-1 left-0 right-0 h-[2px] bg-primary rounded-full"></span>}
                        </Link>
                    </li>
                </ul>

                {/* Auth / Avatar / Notifications / Mobile Menu */}
                    ...
                    {user && (
                        <div className="flex items-center gap-4">
                            ...
                            <div className="relative" ref={notifRef}>
                                <Button variant="ghost" onClick={() => setNotifOpen(!notifOpen)} className="relative hover:text-primary">
                                    <Bell size={20} />
                                    {unreadCount > 0 && (
                                ...
                                {notifOpen && (
                                    <div className="absolute -right-16 md:-right-36 lg:-right-28 mt-2 w-80 bg-white shadow-lg rounded-lg z-50 text-right max-h-96 overflow-y-auto">
                                        <div className="flex justify-between items-center px-4 py-2 border-b">
                                            ...
                                            <Button onClick={markAllAsRead} variant="outline" className="text-sm font-medium text-primary px-3 py-1 hover:bg-primary hover:text-white transition-all bg-white">
                                                وضع الكل كمقروء
                                            </Button>
                                        </div>
                                        {notifications.length === 0 && <p className="px-4 py-2 text-sm text-gray-500">لا توجد إشعارات</p>}
                                        {notifications.map(n => (
                                            <div key={n.id} className={`px-4 py-2 cursor-pointer hover:bg-gray-100 ${n.read ? 'bg-gray-50' : 'bg-white font-semibold'}`}
                                                onClick={() => {
                                                    markAsRead(n.id);
                            ...
                            <div className="relative block" ref={avatarRef}>
                                <div className="flex items-center gap-2 cursor-pointer select-none" onClick={() => setAvatarOpen(!avatarOpen)}>
                                    <Avatar
                                        url={user.avatar ? (user.avatar.startsWith("http") ? user.avatar : `${process.env.NEXT_PUBLIC_BASE_URL}/assets/${user.avatar}`) : null}
